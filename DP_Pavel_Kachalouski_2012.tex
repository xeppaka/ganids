...
\acknowledgements{}
\abstractEN{The goal of this thesis is to develop and test application for network intrusion detection accelerated with GPU. Deep packet inspection is a heavy process and consumes a lot of CPU calculation resources, thereby the main application objective is to offload the heaviest operations during analysing network traffic to the GPU. The project is based on Nvidia CUDA architecture and uses standard features provided by CUDA devices with 1.1 version of compute capabilities.}
\abstractCS{Cílem této práce je vývoj a testování aplikaci pro detekci průniků sítě zrychlené pomoci GPU. Hloubkové prověřování paketů je těžký proces který konzumuje velké množství výpočetních prostředků procesoru, tím hlavním cílem aplikace je zbavit nejtěžší operace v analýze síťového provozu na GPU. Projekt je založen na architektuře NVIDIA CUDA a používá standardní funkce poskytované CUDA zařízení s verzí 1.1 výpočetních schopností.}
\placeForDeclarationOfAuthenticity{Prague}
\keywordsCS{Síťová analýza, Detekce průniku, GPU, CUDA, Paralelní výpočty.}
\keywordsEN{Network analysis, Intrusion detection, GPU, CUDA, Parallel computing.}
...
\lstset{language=C++, label=lst:main_window, captionpos=b, caption={GanidsMainWindow class.}}
\begin{lstlisting}
